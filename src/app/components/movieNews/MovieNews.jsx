...
const MovieNews = () => {
  ...
  const firstArticleWithMultimedia = Array.isArray(moviesArticles) && moviesArticles.find((article) => article.multimedia && article.multimedia.length > 0)

  return (
    <div className='max-w-7xl mx-auto'>
      <h1 className={`${noticia.className} text-3xl px-4 md:text-4xl 2xl:text-5xl`}>Movie News</h1>
      <h2 className={`${noticia.className} px-4 md:text-lg 2xl:text-3xl`}>See what&apos;s happening on the silver screen</h2>
          ...
          <Fade key={index} direction='up' cascade triggerOnce>
            <div key={index} className='bg-white rounded-lg shadow-md p-4'>
              {moviesArticle.multimedia && moviesArticle.multimedia.length > 0 && (
                <Image
                  src={moviesArticle.multimedia[0].url}
                  alt={moviesArticle.multimedia[0].caption}
                  width={moviesArticle.multimedia[0].width}
                  height={moviesArticle.multimedia[0].height}
                  className='rounded-lg shadow-sm shadow-gray-500 w-full transition-all duration-500 hover:scale-105'
                  priority
                />
              )}
              <p className='text-xs italic'>{moviesArticle.byline.replace('By', '').trim()}&nbsp;<span>{new Date(moviesArticle.published_date).toLocaleDateString()}</span></p>
              <div className='flex flex-col'>
                <h2 className='text-lg font-bold'>{moviesArticle.title}</h2>
                <p className='text-gray-600'>{moviesArticle.abstract}</p>
                <Link href={moviesArticle.url} target="_blank" rel='noopener noreferrer' className='text-blue-500 hover:opacity-70 hover:underline text-sm'>Read more on NYT</Link>
              </div>
            </div>
          </Fade>
        ))}
