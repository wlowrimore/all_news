...
const HealthNews = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch(`https://api.nytimes.com/svc/topstories/v2/health.json?api-key=${API_KEY}`, { 'no-cache': true })
      const data = await response.json()
      setHealthArticles(data.results)
      console.log(data)
    }
    fetchData()
  }, [])
  ...
  const firstArticleWithMultimedia = Array.isArray(healthArticles) && healthArticles.find((article) => article.multimedia && article.multimedia.length > 0)

  return (
    <div className='max-w-7xl mx-auto'>
      <h1 className={`${noticia.className} text-3xl px-4 md:text-4xl 2xl:text-5xl`}>Health News</h1>
      <h2 className={`${noticia.className} px-4 md:text-lg 2xl:text-3xl`}>Keeping the world healthy</h2>
          ...
          <Fade key={index} direction='up' cascade triggerOnce>
            <div key={index} className='bg-white rounded-lg shadow-md p-4'>
              {healthArticle.multimedia && healthArticle.multimedia.length > 0 ? (
                <Image
                  src={healthArticle.multimedia[0].url}
                  alt={healthArticle.multimedia[0].caption}
                  width={healthArticle.multimedia[0].width}
                  height={healthArticle.multimedia[0].height}
                  className='rounded-lg shadow-sm shadow-gray-500 w-full transition-all duration-500 hover:scale-105'
                  priority
                />
              ) : null}
              <p className='text-xs italic'>{healthArticle.byline.replace('By', '').trim()}&nbsp;<span>{new Date(healthArticle.published_date).toLocaleDateString()}</span></p>
              <div className='flex flex-col'>
                <h2 className='text-lg font-bold'>{healthArticle.title}</h2>
                <p className='text-gray-600'>{healthArticle.abstract}</p>
                <Link href={healthArticle.url} target="_blank" rel='noopener noreferrer' className='text-blue-500 hover:opacity-70 hover:underline text-sm'>Read more on NYT</Link>
              </div>
            </div>
          </Fade>
        ))}
