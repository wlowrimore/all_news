...
const MagazineNews = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch(`https://api.nytimes.com/svc/topstories/v2/magazine.json?api-key=${API_KEY}`, { 'no-cache': true })
      const data = await response.json()
      setMagazineArticles(data.results)
      console.log(data)
    }
    fetchData()
  }, [])
  ...
  const firstArticleWithMultimedia = Array.isArray(magazineArticles) && magazineArticles.find((article) => article.multimedia && article.multimedia.length > 0);

  return (
    <div className='max-w-7xl mx-auto'>
      <h1 className={`${noticia.className} text-3xl px-4 md:text-4xl 2xl:text-5xl`}>Magazine News</h1>
      <h2 className={`${noticia.className} px-4 md:text-lg 2xl:text-3xl`}>Intresting Articles on various topics</h2>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
        ...
        {magazineArticles && magazineArticles.filter((magazineArticle) => magazineArticle.multimedia && magazineArticle.multimedia.length > 0).map((magazineArticle, index) => (
          <Fade direction='up' cascade triggerOnce key={index}>
            <div key={index} className='bg-white rounded-lg shadow-md p-4'>
              {magazineArticle.multimedia && magazineArticle.multimedia.length > 0 && (
                <Image
                  src={magazineArticle.multimedia[0].url}
                  alt={magazineArticle.multimedia[0].caption}
                  width={magazineArticle.multimedia[0].width}
                  height={magazineArticle.multimedia[0].height}
                  className='rounded-lg shadow-sm shadow-gray-500 w-full transition-all duration-500 hover:scale-105'
                  priority
                />
              )}
              <p className='text-xs italic'>{magazineArticle.byline.replace('By', '').trim()}&nbsp;<span>{new Date(magazineArticle.published_date).toLocaleDateString()}</span></p>
              <div className='flex flex-col'>
                <h2 className='text-lg font-bold'>{magazineArticle.title}</h2>
                <p className='text-gray-600'>{magazineArticle.abstract}</p>
                <Link href={magazineArticle.url} target="_blank" rel='noopener noreferrer' className='text-blue-500 hover:opacity-70 hover:underline text-sm'>Read more on NYT</Link>
              </div>
            </div>
          </Fade>
        ))}
