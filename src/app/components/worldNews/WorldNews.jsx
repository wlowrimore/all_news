...
const WorldNews = () => {
  ...
  const firstArticleWithMultimedia = Array.isArray(worldArticles) && worldArticles.find((article) => article.multimedia && article.multimedia.length > 0)
        ...
        {worldArticles && worldArticles.filter((worldArticle) => worldArticle.multimedia && worldArticle.multimedia.length > 0).map((worldArticle, index) => (
          <Fade key={index} direction='up' cascade triggerOnce>
            <div key={index} className='bg-white rounded-lg shadow-md p-4'>
              ...
              <p className='text-xs italic'>{worldArticle.byline.replace('By', '').trim()}&nbsp;<span>{new Date(worldArticle.published_date).toLocaleDateString()}</span></p>
